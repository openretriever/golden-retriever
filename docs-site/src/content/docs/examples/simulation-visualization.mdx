...
`demo-drawer-mock` runs a second simulation lane: a Panda on an Omron
...
![four stages of the routine: lining up on the handle, gripping it, the drawer pulled open with the spice jars inside, and shut again with the worktop cleared](../../../assets/media/robocasa-drawer/robocasa-drawer-stages.jpg)
...
`demo-drawer-verify` is what actually asserts the claim — no actuator on
