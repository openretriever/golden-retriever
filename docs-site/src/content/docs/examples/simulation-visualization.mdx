...
3. **Mock RoboCasa contract** — `retriever run demo-robocasa-mock` verifies the recorded-action → simulator → observation graph without RoboCasa.
4. **Real RoboCasa / Rerun** — replay a human demonstration through real RoboCasa physics after the mock trace looks right.
5. **Real robosuite / Rerun** — swap `mode="robosuite"` (optional dependency) only after the mock trace looks right.
