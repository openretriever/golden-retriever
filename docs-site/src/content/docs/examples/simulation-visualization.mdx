...
mobile base takes hold of a drawer handle and pulls the drawer open, with two
loose spice jars rolling around inside it.

![four stages: lining up on the handle, gripping it, the drawer pulled out with two spice jars rolling inside, and pushed shut again](../../../assets/media/drawer-dash/drawer-dash-stages.jpg)
...
open unless something is physically holding it. The routine squares up to the
handle bar, closes the fingers across it, and then drives the *hand* out by
41 cm; the drawer follows because the fingers are on it.
...
The nine-phase choreography lives in `plan.py` as plain data with no MuJoCo or
RoboCasa import, so the mock lane, the tests and the simulator all replay one
schedule rather than three that have to be kept in step. The mock reproduces
the timeline, the commanded travel and the pass thresholds; it does not
reproduce contact. For that, the MuJoCo lane needs the optional simulator
dependencies and RoboCasa's fixture and object asset packs, and
...
push, and the drawer out past 0.35 m and back shut under that grasp alone.
