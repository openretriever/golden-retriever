...
The GoldenRetriever docs site is deployed by **Cloudflare Pages' Git
integration** — Cloudflare watches this repository and builds/deploys on every
push. There is **no deploy token or secret stored in this repo**; the connection
is held on Cloudflare's side, so contributors (including public PRs) can never
leak it.

## One-time setup (Cloudflare dashboard)

Workers & Pages → **Create application** → **Pages** → **Connect to Git** →
select `openretriever/golden-retriever`, then set:
...
npx wrangler pages deploy dist --project-name=retriever-space --branch=<branch>
```
