...
from vlm_utils import VLMPlanner
...
class WebcamFlow(Flow[None, ImageMsg]):
    ...
    def init(self):
        ...
        rr.init("demo_vlm_planning", spawn=False)
    ...
    <html>
        <body style="font-family: sans-serif; padding: 20px; background: #f4f4f9;">
            <div style="max-width: 600px; margin: auto; background: white; padding: 30px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                <h1 style="color: #333;">Retriever VLM Planning</h1>
...
class VLMReasoningFlow(Flow[ReasoningInput, TextMsg]):
    ...
    def init(self):
        # Create VLMPlanner in subprocess to avoid pickling issues
        self.planner = VLMPlanner(model_name=self._model_name)
        print(f"[VLMReasoningFlow] Using model: {self._model_name}")
        # Connect subprocess to spawned Rerun viewer
        rr.init("demo_vlm_planning", spawn=False)
        rr.connect_grpc()  # Connect to gRPC server on default port
    ...
    def step(self, flow_input: ReasoningInput) -> Optional[TextMsg]:
        ...
        # Log latency metrics
        # BarChart expects a numpy array.
        import numpy as np
        rr.log("metrics/vlm_latency_ms", rr.BarChart(np.array([latency_ms])))
...
"""
            ...
            # Clear old separate logs if they exist (by logging empty? No, simple overriding usage pattern is fine)
            # We just stop logging to the old paths.
            ...
            try:
                ...
                detections = json.loads(coords) if coords else []
                if detections:
...
if __name__ == "__main__":
    ...
    rr.init("demo_vlm_planning", spawn=True)
    retriever.init("demo_planning_stream")

    with Pipeline("demo_vlm_planning") as pipe:
