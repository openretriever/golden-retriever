...
across the handle bar rather than merely near it.

It is deliberately not a robosuite controller: no nullspace posture, no
impedance, no force limits beyond the actuators' own.
"""
...
class Arm:
    ...
    def reach(self, point, rotation=None, dt: float | None = None) -> float:
        ...
        joint rates through a damped least-squares inverse, and the rates
        integrated into the joint targets. Call it every tick — including while
        the target is moving, which is how the arm carries a drawer out.
        ...
        velocity = np.zeros(3)
        if distance > 1e-9:
            velocity = error / distance * min(TIP_SPEED, APPROACH_GAIN * distance)
        ...
        if rotation is None:
            ...
            spin = np.zeros(3)
            if angle > 1e-9:
                spin = turn / angle * min(SPIN_SPEED, ALIGN_GAIN * angle)
            jac = np.vstack([self._jacp[:, self.dof_ids], self._jacr[:, self.dof_ids]])
            twist = np.concatenate([velocity, spin])

        # dq/dt = J^T (J J^T + lambda^2 I)^-1 twist
        lhs = jac @ jac.T + DAMPING ** 2 * np.eye(jac.shape[0])
        rate = np.clip(jac.T @ np.linalg.solve(lhs, twist), -JOINT_SPEED, JOINT_SPEED)
