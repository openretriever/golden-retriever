"""
Stateful replanning with internal planner memory.

Run:
  pixi run demo-stateful-replanning
"""
...
def build_pipeline(*, dt: float) -> Pipeline:
    pipe = Pipeline("golden_stateful_replanning")
    hz = 1.0 / max(dt, 1e-6)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Stateful replanning demo (Golden advanced memory example).")
