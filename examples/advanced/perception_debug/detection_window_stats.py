"""
Windowed detection stats for GoldenRetriever.

Run:
  pixi run demo-detection-window-stats
"""
...
def build_pipeline(*, hz: float, window_s: float, window_buffer_size: int) -> Pipeline:
    pipe = Pipeline("golden_detection_window")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Windowed detection stats (Golden advanced perception demo).")
    ...
    p.add_argument("--window-buffer-size", type=int, default=200)
    return p.parse_args()


def main() -> None:
    args = parse_args()
    pipe = build_pipeline(hz=args.hz, window_s=args.window, window_buffer_size=args.window_buffer_size)
