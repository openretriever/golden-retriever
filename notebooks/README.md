...
`hub_demo.ipynb` is the Hub-first notebook. Run it from the `golden-retriever` environment so the Golden example feature set is available:

```bash
pixi install -e golden-retriever
pixi run -e golden-retriever demo-hub-notebook-source
```
