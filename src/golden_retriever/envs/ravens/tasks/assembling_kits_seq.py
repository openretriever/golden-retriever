    ...
    def reset(self, env):
        ...
        for i in range(n_objects):
            ...
            if list(obj_shapes).count(obj_shapes[i]) > 1:
                duplicate_shapes = [
                    j for j, o in enumerate(obj_shapes) if i != j and o == shape_type
                ]
                other_poses = [
                    utils.apply(kit_pose, targ_pos[d]) for d in duplicate_shapes
                ]

                if all(
                    pos[0] < op[0] and abs(pos[0] - op[0]) > abs(pos[1] - op[1])
