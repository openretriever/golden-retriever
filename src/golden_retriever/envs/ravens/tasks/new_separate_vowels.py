...
class SeparateVowels(Task):
    def __init__(self):
        ...
        self.task_completed_desc = (
            "done sorting the vowels from the remaining letters to the bottom side."
        )
    ...
    def reset(self, env):
            ...
            if i == 0:
                self.lang_goals.append(
                    self.lang_template.format(
                        pick=vowel_letter_names[i], place="bottom side"
                    )
                )
            else:
                self.lang_goals.append(
                    self.lang_template.format(
                        pick=vowel_letter_names[i],
        ...
        self.high_level_lang_goal = (
            "sort the vowels from the remaining letters to the bottom side"
        )

    def success(self):
        letter_poses = [
            p.getBasePositionAndOrientation(lid) for lid, _ in self.vowel_letters
        ]
        matches = [
            self.is_match(letter_poses[i], self.targets[i], symmetry=0)
            for i in range(len(self.vowel_letters))
        ]
        return all(matches)
