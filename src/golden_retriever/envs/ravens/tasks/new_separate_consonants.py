...
class SeparateConsonants(Task):
    def __init__(self):
        ...
        self.task_completed_desc = (
            "done sorting the consonants from the remaining letters to the bottom side."
        )
    ...
    def reset(self, env):
        ...
        consonant_letter_idx = np.sort(
            random.sample(range(len(consonants)), n_consonant_letters)
        )
        ...
        for i in range(n_consonant_letters):
            self.goals.append(
                (
                    ...
                    "pose",
                    None,
                    1 / n_consonant_letters,
                )
            )
            if i == 0:
                self.lang_goals.append(
                    self.lang_template.format(
                        pick=consonant_letter_names[i], place="bottom side"
                    )
                )
            else:
                self.lang_goals.append(
                    self.lang_template.format(
                        pick=consonant_letter_names[i],
        ...
        self.high_level_lang_goal = (
            "sort the consonants from the remaining letters to the bottom side"
        )

    def success(self):
        letter_poses = [
            p.getBasePositionAndOrientation(lid) for lid, _ in self.consonant_letters
        ]
        matches = [
            self.is_match(letter_poses[i], self.targets[i], symmetry=0)
            for i in range(len(self.consonant_letters))
        ]
        return all(matches)
