...
class SortSymmetricalLetters(Task):
    ...
    def reset(self, env):
        ...
        symmetrical_letter_idx = np.sort(
            random.sample(range(len(all_symmetrical_letters)), n_symmetrical_letters)
        )
        symmetrical_letters_letter_names = [
            all_symmetrical_letters[i] for i in symmetrical_letter_idx
        ]
        distractor_letter_names = [
            l for l in alphabet if l not in all_symmetrical_letters
        ]
        distractor_letter_names = random.sample(
            distractor_letter_names, n_distractor_letters
        )
        ...
        targets = [
            (utils.apply(bottom_side_pose, relative_pos[i]), rot)
            for i in range(n_symmetrical_letters)
        ]
        ...
        for i in range(n_symmetrical_letters):
            self.goals.append(
                (
                    ...
                    "pose",
                    None,
                    1 / n_symmetrical_letters,
                )
            )
            if i == 0:
                self.lang_goals.append(
                    self.lang_template.format(
                        pick=symmetrical_letters_letter_names[i], place="bottom side"
                    )
                )
            else:
                self.lang_goals.append(
                    self.lang_template.format(
                        pick=symmetrical_letters_letter_names[i],
        ...
        self.high_level_lang_goal = (
            "sort the geometrically vertically symmetrical letters to the bottom side"
        )

    def success(self):
        letter_poses = [
            p.getBasePositionAndOrientation(lid) for lid, _ in self.symmetrical_letters
        ]
        matches = [
            self.is_match(letter_poses[i], self.targets[i], symmetry=0)
            for i in range(len(self.symmetrical_letters))
        ]
        return all(matches)
