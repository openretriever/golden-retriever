...
class SortLettersLessThanD(Task):
    ...
    def reset(self, env):
        ...
        less_than_d_letter_idx = np.sort(
            random.sample(range(len(all_less_than_d_letters)), n_less_than_d_letters)
        )
        less_than_d_letter_names = [
            all_less_than_d_letters[i] for i in less_than_d_letter_idx
        ]
        distractor_letter_names = [
            l for l in alphabet if l not in all_less_than_d_letters
        ]
        ...
        for i in range(n_less_than_d_letters):
            self.goals.append(
                (
                    ...
                    "pose",
                    None,
                    1 / n_less_than_d_letters,
                )
            )
            if i == 0:
                self.lang_goals.append(
                    self.lang_template.format(
                        pick=less_than_d_letter_names[i], place="bottom side"
                    )
                )
            else:
                self.lang_goals.append(
                    self.lang_template.format(
                        pick=less_than_d_letter_names[i],
    ...
    def success(self):
        letter_poses = [
            p.getBasePositionAndOrientation(lid) for lid, _ in self.less_than_d_letters
        ]
        matches = [
            self.is_match(letter_poses[i], self.targets[i], symmetry=0)
            for i in range(len(self.less_than_d_letters))
        ]
        return all(matches)
