...
class SpellSport(Task):
    def __init__(self):
        ...
        self.task_completed_desc = (
            "done spelling out a sport using the present letters."
        )
            ...
            if i == 0:
                self.lang_goals.append(
                    self.lang_template.format(
                        pick=sport_letter_names[i], place="bottom left corner"
                    )
                )
            else:
                self.lang_goals.append(
                    self.lang_template.format(
                        pick=sport_letter_names[i],
    ...
    def success(self):
        letter_poses = [
            p.getBasePositionAndOrientation(lid) for lid, _ in self.sport_letters
        ]
        matches = [
            self.is_match(letter_poses[i], self.targets[i], symmetry=0)
            for i in range(len(self.sport_letters))
        ]
        return all(matches)
