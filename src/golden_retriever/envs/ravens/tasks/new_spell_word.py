...
class SpellWord(Task):
    ...
    def reset(self, env):
        ...
        subword_letters_idx = np.sort(
            random.sample(range(len(word)), n_subword_letters)
        )
            ...
            if i == 0:
                self.lang_goals.append(
                    self.lang_template.format(
                        pick=subword_letter_names[i], place="bottom left corner"
                    )
                )
            else:
                self.lang_goals.append(
                    self.lang_template.format(
                        pick=subword_letter_names[i],
    ...
    def success(self):
        letter_poses = [
            p.getBasePositionAndOrientation(lid) for lid, _ in self.subword_letters
        ]
        matches = [
            self.is_match(letter_poses[i], self.targets[i], symmetry=0)
            for i in range(len(self.subword_letters))
        ]
        return all(matches)
