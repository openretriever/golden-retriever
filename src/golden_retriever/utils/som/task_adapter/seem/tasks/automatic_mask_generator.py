...
class SeemAutomaticMaskGenerator:
    ...
    def _process_batch(
        ...
        abs_points = (
            points * torch.tensor(orig_size)[None, :].to(points.device)
        ).long()
        abs_masks = torch.zeros((len(points), orig_h, orig_w), dtype=torch.bool).to(
            device=points.device
        )
        abs_masks[
            torch.arange(0, abs_points.size(0))[:, None],
            abs_points[:, 0:1],
            abs_points[:, 1:2],
        ] = True
        ...
        if self.enc_features is None:
            (
                ...
                batch_inputs, None, None, return_features=True
            )
            self.enc_features = (
                mask_features,
                transformer_encoder_features,
