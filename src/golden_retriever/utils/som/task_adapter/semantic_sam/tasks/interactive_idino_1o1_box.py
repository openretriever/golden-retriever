...
def interactive_infer_image_box(
    model,
    image,
    all_classes,
    all_parts,
    ...
    if len(points) == 0:
        ...
        w = mask_ori.shape[1]
        box_xywh = (
            box_ops.box_xyxy_to_cxcywh(box_xyxy) / torch.as_tensor([w, h, w, h])
        ).cuda()
    ...
    batch_inputs = [data]
    masks, ious = model.model.evaluate_demo(
        batch_inputs, all_classes, all_parts, task="demo_box"
    )
