...
def interactive_infer_image(
    ...
    for i, (pred_masks_pos, iou) in enumerate(
        zip(pred_masks_poses[ids], ious[ids], strict=False)
    ):
...
def interactive_infer_image_3l(
    model,
    image,
    all_classes,
    all_parts,
    ...
    batch_inputs = [data]
    masks, ious, pred_class, pred_class_score = model.model.evaluate_demo(
        batch_inputs, all_classes, all_parts, level=[0, 1, 2]
    )
    ...
    for i, (pred_masks_pos, iou, cls_name, cls_score) in enumerate(
        zip(
            pred_masks_poses[ids],
            ious[ids],
            new_pred_class,
            new_pred_class_score,
            strict=False,
...
def interactive_infer_image_semantic(
    model,
    image,
    all_classes,
    all_parts,
    ...
    data["targets"][0]["points"] = point
    data["targets"][0]["pb"] = point.new_tensor([0.0])
    data["targets"][0]["pb"] = point.new_tensor([1.0])
