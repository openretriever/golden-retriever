...
def sam_interactive_mask(mask_generator, points, in_points, in_labels, mask_input):
    ...
    data["stability_score"] = calculate_stability_score(
        data["masks"],
        mask_generator.predictor.model.mask_threshold,
        mask_generator.stability_score_offset,
    )
...
def inference_sam_m2m_interactive(
    model,
    image,
    spatial_masks,
    text_size,
    ...
    spatial_masks = (
        F.interpolate(
            spatial_masks, size=(orig_h, orig_w), mode="bicubic", align_corners=False
        )
        > 0
    )
    ...
    transformed_points = mask_generator.predictor.transform.apply_coords(
        points, im_size
    )
    in_points = (
        torch.as_tensor(transformed_points, device=mask_generator.predictor.device)
        .reshape(_np, -1, 2)
        .transpose(0, 1)
    )
    in_labels = torch.ones(
        (in_points.shape[0], _np),
        dtype=torch.int,
        device=mask_generator.predictor.device,
    )

    masks = sam_interactive_mask(
        mask_generator,
        points,
        in_points.transpose(0, 1),
        in_labels.transpose(0, 1),
        None,
    )
    ...
    mask_data["crop_boxes"] = torch.tensor(
        [crop_box for _ in range(len(mask_data["boxes"]))]
    )
    ...
    from retriever.utils.som.task_adapter.utils.visualizer import Visualizer
