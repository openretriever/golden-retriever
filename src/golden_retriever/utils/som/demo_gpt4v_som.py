...
from task_adapter.seem.tasks import inference_seem_interactive, inference_seem_pano
...
semsam_ckpt = assets_root + "/downloaddata/swinl_only_sam_many2many.pth"
sam_ckpt = assets_root + "/downloaddata/sam_vit_h_4b8939.pth"
seem_ckpt = assets_root + "/downloaddata/seem_focall_v1.pt"
...
demo = gr.Blocks()
image = ImageMask(
    label="Input", type="pil", brush_radius=20.0, brush_color="#FFFFFF", height=512
)
# image = gr.Image(label="Input", type="pil", height=512)

slider = gr.Slider(
    1, 3, value=1.8, label="Granularity"
...
anno_mode = gr.CheckboxGroup(
    choices=["Mark", "Mask", "Box"], value=["Mark"], label="Annotation Mode"
)
...
slider_alpha = gr.Slider(
    0, 1, value=0.05, label="Mask Alpha"
)  # info="Choose in [0, 1]"
label_mode = gr.Radio(["Number", "Alphabet"], value="Number", label="Mark Mode")

title = (
    "Set-of-Mark (SoM) Visual Prompting for Extraordinary Visual Grounding in GPT-4V"
)
...
with demo:
    ...
    runBtn.click(
        inference,
        inputs=[image, slider, mode, slider_alpha, label_mode, anno_mode],
        outputs=image_out,
    )
    highlightBtn.click(
        highlight,
        inputs=[image, mode, slider_alpha, label_mode, anno_mode],
        outputs=image_out,
    )
