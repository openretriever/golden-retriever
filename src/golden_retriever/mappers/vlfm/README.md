...
## The below documentation has been updated to reflect the upgrades and changes made to the codebase of VLFM in Retriever. The original documentation can be found [here](https://github.com/bdaiinstitute/vlfm/blob/main/README.md)
...
git clone https://github.com/naokiyokoyama/bd_spot_wrapper.git # If in retriever use the version in the external branch
```
...
The weights for PointNav must be saved to the `data/` directory:
...
## When running: run from root directory (`Retriever/`)

## :arrow_forward: Evaluation within Habitat

Run the following to evaluate on the HM3D dataset:
```bash
python -m src.mappers.vlfm.vlfm.run
```
To run the evaluation and save the video of the simulator:
```
python -m src.mappers.vlfm.vlfm.run habitat_baselines.eval.video_option='["disk"]'
```
To evaluate on MP3D, run the following:
```bash
python -m src.mappers.vlfm.vlfm.run habitat.dataset.data_path=data/datasets/objectnav/mp3d/val/val.json.gz
```
...
python -m src.mappers.vlfm.vlfm.reality.run_bdsw_objnav_env env.goal=<OBJECT_NAME>
```

## Ray hosting externally

To host on ray locally program can be run without any additional arguments or instructions. To host on ray externally, the following example can be followed:

On host machine:
```bash
# Make sure same ray version and python version as client machine
ray start --head
```
...
# For windows powershell alternate check scripts/mappers/vlfm/ray.ps1
export RAY_ADDRESS=ray://<HOST_IP>:<HOST_PORT>
# Example: 'ray://grail-mercury.neu.edu:10001'
