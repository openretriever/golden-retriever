...
class HabitatConfigPlugin(SearchPathPlugin):
    def manipulate_search_path(self, search_path: ConfigSearchPath) -> None:
        search_path.append(provider="habitat", path="src/config/mappers/vlfm")
...
@hydra.main(
    version_base=None,
    config_path="../../../config/mappers/vlfm",
    config_name="experiments/vlfm_objectnav_hm3d",
)
def main(cfg: DictConfig) -> None:
    assert os.path.isdir("src/mappers/vlfm/data"), "Missing 'data/' directory!"
    if not os.path.isfile("src/mappers/vlfm/data/dummy_policy.pth"):
        print("Dummy policy weights not found! Please run the following command first:")
        print("python -m mappers.vlfm.vlfm.utils.generate_dummy_policy")
        exit(1)
