...
from retriever.models.segmentation.mobilesam_actor import MobileSAMActor
...
from ..vlm.grounding_dino import GroundingDINOClientRay, ObjectDetections
from ..vlm.sam import MobileSAMClientRay
from ..vlm.yolov7 import YOLOClientRay

# pg = placement_group([{"GPU": 1}])


# from .....models.detection.groundingdino_actor import GroundingDinoActor
# from .....models.detection.yolo_actor import YOLOActor
# from .....models.segmentation.mobilesam_actor import MobileSAMActor
...
class BaseObjectNavPolicy(BasePolicy):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        gdino_actor = GroundingDinoActor.options(**actor_options).remote(
            use_gpu=use_gpu
        )
        yolo_actor = YOLOActor.options(**actor_options).remote(
            use_gpu=use_gpu, model_name="yolo11n.pt"
        )
        mobile_sam_actor = MobileSAMActor.options(**actor_options).remote(
            use_gpu=use_gpu
        )

        # gdino_actor = GroundingDinoActor.options(scheduling_strategy = strat).remote(use_gpu=use_gpu)
        # yolo_actor = YOLOActor.options(scheduling_strategy = strat).remote(use_gpu=use_gpu, model_name="yolo11n.pt")
        # mobile_sam_actor = MobileSAMActor.options(scheduling_strategy = strat).remote(use_gpu=use_gpu)

        self._object_detector = GroundingDINOClientRay(gdino_actor)

        # self._coco_object_detector = YOLOv7Client(port=int(os.environ.get("YOLOV7_PORT", "12184")))

        self._coco_object_detector = YOLOClientRay(yolo_actor)

        # self._mobile_sam = MobileSAMClient(port=int(os.environ.get("SAM_PORT", "12183")))

        self._mobile_sam = MobileSAMClientRay(mobile_sam_actor)
...
class VLFMConfig:
    ...
    pointnav_policy_path: str = "src/mappers/vlfm/data/pointnav_weights.pth"
