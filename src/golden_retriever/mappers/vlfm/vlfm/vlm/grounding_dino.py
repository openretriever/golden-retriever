...
class GroundingDINOClientRay:
    def __init__(self, actor_handle):
        # The actor_handle is the reference to the Ray actor
        self.actor_handle = actor_handle

    def predict(self, image_numpy: np.ndarray, caption: Optional[str] = ""):
        # Convert the image numpy array to PIL image if needed
        image = Image.fromarray(image_numpy)

        # Make the remote call to the actor's predict method
        result = ray.get(self.actor_handle.predict.remote(image, caption))

        if len(result) == 1:
            result = result[0]
            if "labels" in result:
                result["phrases"] = result.pop("labels")
            if "scores" in result:
                result["logits"] = result.pop("scores")

            detections = ObjectDetections.from_json(result, image_source=image_numpy)
        else:
            detections = []
            for res in result:
                detections.append(
                    ObjectDetections.from_json(res, image_source=image_numpy)
                )
