...
sys.path.insert(0, "src/mappers/vlfm/yolov7/")
try:
...
class YOLOClientRay:
    ...
    def predict(self, image_numpy: np.ndarray) -> ObjectDetections:
        ...
        result = ray.get(self.actor_handle.predict.remote(image))

        if len(result) == 1:
            result = result[0]
            boxes = result.cpu().boxes.xyxy
            logits = result.cpu().boxes.conf
            phrases = [COCO_CLASSES[int(i)] for i in result.cpu().boxes.cls]
            detections = ObjectDetections(
                boxes, logits, phrases, image_source=image, fmt="xyxy"
            )
        else:
