`graph_nav_command_line.py` used to localize against a Graph Nav map and navigate on it

`recording_command_line.py` used to record new Graph Nav maps
...
```
python -m src.mappers.utils_graphnav.recording_command_line --download-filepath <path_to_downloaded_map> ROBOT_IP
```
...
Option _9_ allows the recording service to automatically identify and close loops (including at the start and end of the map), using fiducials, odometry, and other methods

Option _a_ creates an _anchoring_ for the map, which allows it to be more accurately drawn and used for data export
...
```
python -m src.mappers.utils_graphnav.graph_nav_command_line ROBOT_IP --upload-filepath "src/robots/spot/graph_nav_maps/exp_floor_1"
```
