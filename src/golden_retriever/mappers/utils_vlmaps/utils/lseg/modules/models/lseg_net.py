...
class LSegEnc(BaseModel):
    def __init__(
        ...
        **kwargs,
        ...
        clip_pretrained, self.pretrained, self.scratch = _make_encoder(
            backbone,
            features,
            groups=1,
            expand=False,
        ...
        if kwargs.get("load_text_encoder", True):
        ...
        self.logit_scale = (
            nn.Parameter(torch.ones([]) * np.log(1 / 0.07)).exp().to(self.device)
        )
    ...
    def forward(self, x: Tensor, labelset: str) -> Tuple[Tensor]:
        # extract text features
        text_features = self.extract_text_features(labelset)

        # extract image features
        image_features = self.extract_img_features(x, upsample=False)

        # permute from B x C x H x W -->  B x H x W x C
        ...
        # compute the logits per pixel
        logits_per_image = pixel_encoding @ text_features.t()

        # permute from B x C x H x W --> B x H x W x C
        pixel_encoding = (
            pixel_encoding.float()
            .view(im_shape[0], im_shape[2], im_shape[3], -1)
            .permute(0, 3, 1, 2)
        )
    ...
    def extract_img_features(self, x: Tensor, upsample=True) -> Tensor:
    ...
    def extract_text_features(self, labelset):
        if labelset == "":
            text = self.text
        else:
