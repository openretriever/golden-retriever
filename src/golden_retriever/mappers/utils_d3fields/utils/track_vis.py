    ...
    def visualize_match_pts(self, match_pts_list, full_pcd, colors, track_info):
        ...
        if self.vis_o3d:
                ...
                for k_i, k in enumerate(track_info.keys()):
                    for sphere_i in range(self.ptcl_num):
                        sphere = o3d.geometry.TriangleMesh.create_sphere(
                            radius=0.005, resolution=10, create_uv_map=True
                        )
                        sphere.compute_vertex_normals()
                        sphere.paint_uniform_color(
                            track_info[k]["src_pts_color"][sphere_i, ::-1] / 255.0
                        )
            ...
            for sphere_i in range(len(self.sphere_list)):
                self.sphere_list[sphere_i].vertices = o3d.utility.Vector3dVector(
                    self.base_sphere_vertex
                    + match_pts_list[sphere_i // self.ptcl_num][
                        sphere_i % self.ptcl_num
                    ]
                )
            ...
            view_control.set_front(
                [-0.3091336095239805, 0.10370358297217445, -0.94534754367978802]
            )
            view_control.set_lookat(
                [0.10592095570082928, 0.039109756311030912, 0.1852527727811189]
            )
            view_control.set_up(
                [-0.022245326124056605, 0.9929763673752956, 0.11620275082714933]
            )
            ...
            if self.t == 0:
                ...
                self.o3d_vid = cv2.VideoWriter(
                    self.o3d_vid_path,
                    cv2.VideoWriter_fourcc(*"mp4v"),
                    10,
                    (o3d_img.shape[1], o3d_img.shape[0]),
                )
        ...
        if self.t == 0:
            self.vid = cv2.VideoWriter(
                ...
                10,
                (
                    int(self.W * self.imshow_ratio) * 2,
                    int(self.H * self.imshow_ratio) * 2,
                ),
            )
        merge_img = np.zeros(
            (
        ...
        for view_i in range(self.num_cam):
            h_idx = view_i // 2
            w_idx = view_i % 2
            vis_img = vis_tracking_multimodal_pts(
                colors[view_i][..., ::-1],
                self.Ks[view_i],
                self.poses[view_i],
                match_pts_list,
                preset_colors=np.concatenate(
                    [track_info[k]["src_pts_color"] for k in track_info.keys()], axis=0
                ),
            )
            merge_img[
                h_idx
                * int(self.H * self.imshow_ratio) : (h_idx + 1)
                * int(self.H * self.imshow_ratio),
                w_idx
                * int(self.W * self.imshow_ratio) : (w_idx + 1)
                * int(self.W * self.imshow_ratio),
            ] = cv2.resize(
                vis_img,
                (int(self.W * self.imshow_ratio), int(self.H * self.imshow_ratio)),
