...
def compute_similarity(src_feat_map, tgt_feat, scale, dist_type="l2"):
    ...
    if dist_type == "square":
        ...
        feat_dist = np.linalg.norm(
            src_feat_map - tgt_feat[None, None, None, :], axis=-1
        )
    else:
...
def compute_similarity_tensor_multi(
    src_feat_map, tgt_feats, src_pts, last_match_pts, scale, dist_type="l2"
):
        ...
        try:
            ...
            if "out of memory" in str(e):
                ...
                for i in range(0, src_feat_map.shape[0], batchsize):
                    ...
                    feat_dist_batch = torch.norm(
                        src_feat_map[i:upper] - tgt_feats, dim=2
                    )
                    if i == 0:
