...
def gen_dense_kypts(data_path, src_feat_info):
    colors = np.stack(
        [
            cv2.imread(os.path.join(data_path, f"camera_{i}", "color", "0.png"))
    ...
    depths = (
        np.stack(
            [
                cv2.imread(
                    os.path.join(data_path, f"camera_{i}", "depth", "0.png"),
                    cv2.IMREAD_ANYDEPTH,
                )
                for i in range(num_cam)
            ],
    ...
    query_thresholds = [
        src_feat_info[k]["params"]["sam_threshold"] for k in query_texts
    ]

    # create output dir
    full_pts_path = os.path.join(
        data_path, "obj_kypts"
    )  # list of (ptcl_num, 3) for each push, indexed by push_num
    os.system(f"mkdir -p {full_pts_path}")

    track_vis = TrackVis(
        poses=extrinsics, Ks=intrinsics, output_dir=full_pts_path, vis_o3d=vis_o3d
    )

    time_skip = 1
    times = list(range(len(os.listdir(os.path.join(data_path, "camera_0", "color")))))
    ...
    for t in tqdm(times[::time_skip]):
        ...
        depths = (
            np.stack(
                [
                    cv2.imread(
                        os.path.join(data_path, f"camera_{i}", "depth", f"{t}.png"),
                        cv2.IMREAD_ANYDEPTH,
                    )
                    for i in range(num_cam)
                ],
        ...
        fusion.text_queries_for_inst_mask(
            query_texts, query_thresholds, boundaries=boundaries
        )

        # initialize for tracking
        if t == 0:
            rand_ptcl_num = 100
            src_feats_list, src_pts_list, color_list = fusion.select_features_rand(
                kypts_boundaries, rand_ptcl_num, per_instance=True
            )

            # save src_feats_list and src_pts_list
            src_feats_np_list = [
                src_feats.detach().cpu().numpy() for src_feats in src_feats_list
            ]
            src_pts_np_list = [src_pts for src_pts in src_pts_list]
            pickle.dump(
                src_feats_np_list,
                open(os.path.join(full_pts_path, "src_feats_list.pkl"), "wb"),
            )
            pickle.dump(
                src_pts_np_list,
                open(os.path.join(full_pts_path, "src_pts_list.pkl"), "wb"),
            )

            # save make label
            pickle.dump(
                fusion.curr_obs_torch["mask_label"][0],
                open(os.path.join(full_pts_path, "mask_label.pkl"), "wb"),
            )
            ...
            for k_i, k in enumerate(fusion.curr_obs_torch["mask_label"][0][1:]):
                if k == last_k:
                    ...
                    src_feat_loc_norm = (
                        src_feat_info[k]["src_pts"][:, 0]
                        - src_feat_info[k]["src_pts"][:, 0].min()
                    ) / (
                        src_feat_info[k]["src_pts"][:, 0].max()
                        - src_feat_info[k]["src_pts"][:, 0].min()
                    )
                    cmap = cm.get_cmap("viridis")
                    colors = (cmap(src_feat_loc_norm)[:, :3] * 255).astype(np.uint8)[
                        :, ::-1
                    ]
        ...
        tracking_res = fusion.rigid_tracking(
            src_feat_info, match_pts_list, boundaries, rand_ptcl_num
        )
        match_pts_list = tracking_res["match_pts_list"]

        track_vis.visualize_match_pts(
            match_pts_list, pcd, obs["color"][..., ::-1], src_feat_info
        )

        pickle.dump(
            match_pts_list, open(os.path.join(full_pts_path, f"{t:06d}.pkl"), "wb")
        )


if __name__ == "__main__":
    src_feat_info = {
        "shoe": {"params": {"sam_threshold": 0.6}, "src_feats_path": None},
    }
    gen_dense_kypts(
        "src/models/mapping/utils_d3fields/data/2023-09-14-17-06-38-562096",
        src_feat_info,
    )
