...
img_paths = [
    f"data/dino_pca/{obj_type}/0.png",
...
for i in range(4):
    num_fg_list.append(
        pca_features_fg[i * patch_h * patch_w : (i + 1) * patch_h * patch_w].sum()
    )
pca.fit(fg_features)
...
for i in range(3):
    ...
    for j in range(4):
        ...
        pca_features_rem[start_idx:end_idx, i] = (
            pca_features_rem[start_idx:end_idx, i]
            - pca_features_rem[start_idx:end_idx, i].min()
        ) / (
            pca_features_rem[start_idx:end_idx, i].max()
            - pca_features_rem[start_idx:end_idx, i].min()
        )
        start_idx = end_idx
