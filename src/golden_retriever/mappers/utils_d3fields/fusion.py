...
from utils.draw_utils import aggr_point_cloud_from_data, draw_keypoints
...
class Fusion:
    def __init__(
        ...
        # load XMem model
        XMem_path = os.path.join(curr_path, "external/XMem/saves/XMem.pth")
        if not os.path.exists(XMem_path):
            print("Downloading XMem model...")
            ckpts_dir = os.path.join(curr_path, "external/XMem/saves")
            os.system(f"mkdir -p {ckpts_dir}")
            os.system(
    ...
    def text_queries_for_inst_mask_no_track(
        ...
        for i in range(self.num_cam):
            ...
            mask, label, mask_conf = grounded_instance_sam_new_ver(
    ...
    def text_queries_for_inst_mask(
        ...
        if not self.xmem_first_mask_loaded:
            ...
            self.curr_obs_torch["mask_conf"] = mask_confs
    ...
    def select_features_from_pcd(
        self, pcd, N, per_instance=False, init_idx=-1, vis=False
    ):
        # pcd: (N, 3) numpy array
        dist_threshold = 0.005
