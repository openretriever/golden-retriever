...
from external.XMem.dataset.range_transform import im_normalization
from external.XMem.inference.data.mask_mapper import MaskMapper
from external.XMem.inference.inference_core import InferenceCore
from external.XMem.model.network import XMem
from retriever.mappers.utils_d3fields.utils.grounded_sam import (
    grounded_instance_sam_bacth_queries_np,
)
from retriever.models.mapping.utils_d3fields.utils.corr_utils import (
    compute_similarity_tensor,
    compute_similarity_tensor_multi,
)
from retriever.models.mapping.utils_d3fields.utils.draw_utils import (
    aggr_point_cloud_from_data,
    draw_keypoints,
)
from retriever.models.mapping.utils_d3fields.utils.my_utils import (
    depth2fgpcd,
    depth2normal,
    find_indices,
    fps_np,
...
def vis_tracking_multimodal_pts(
    database, match_pts_list, conf_list, sel_time, mask, view_idx=0
):
    ...
    for i, match_pts in enumerate(match_pts_list):
        ...
        match_pts = np.concatenate(
            [match_pts, np.ones([num_pts, 1])], axis=-1
        )  # [num_pts, 4]
...
def octree_subsample(sim_vol, que_pts, last_res, topK):
    ...
    sim_vol_topk, sim_vol_topk_idx = torch.topk(
        sim_vol, topK, dim=0, largest=True, sorted=False
    )  # [topK, N]
...
def extract_kypts_gpu(sim_vol, que_pts, match_metric="sum"):
    ...
    if match_metric == "max":
        ...
        # vectorized version
        match_pts_tensor = torch.sum(
            que_pts_tensor.unsqueeze(1) * sim_vol_tensor.unsqueeze(-1), dim=0
        ) / torch.sum(sim_vol_tensor, dim=0).unsqueeze(
            -1
        )  # [N, 3]
...
class Fusion:
    def __init__(self, num_cam, feat_backbone="dinov2"):
        ...
        if self.feat_backbone == "dinov2":
            ...
            self.feat_extractor = ViTExtractor(
                model_type="dino_vitb8", stride=8, device=self.device
            )
        ...
        self.ground_dino_model = load_model(
    ...
    # TODO the core function to compute features
    def eval(self, pts, return_names=["dino_feats", "mask"], return_inter=False):
        ...
        inter_normal = interpolate_feats(
            self.curr_obs_torch["normals"].permute(0, 3, 1, 2),
            pts_2d,
            h=self.H,
            w=self.W,
            padding_mode="zeros",
    ...
    # TODO call 3: out = fusion.batch_eval(init_grid, return_names=[])
    # TODO call 5: out = fusion.batch_eval(vertices_tensor, return_names=['dino_feats', 'mask', 'color_tensor'])
        ...
        if self.feat_backbone == "dinov2":
            ...
            with torch.no_grad():
                ...
                for i in range(K):
                    ...
                    features.append(
                        self.feat_extractor.forward_single_image_tensor(imgs_tensor[i])
                    )
    ...
    def update(self, obs):
        ...
        if self.feat_backbone == "dinov2":
            ...
            self.curr_obs_torch["dino_v2_feats"] = self.extract_dinov2_features(
                color, params
            )
        self.curr_obs_torch["color"] = color
        normals_np = [
            depth2normal(obs["depth"][i], obs["K"][i]) for i in range(self.num_cam)
        ]
        normals_np = np.stack(normals_np, axis=0)
        self.curr_obs_torch["normals"] = torch.from_numpy(normals_np).to(
            self.device, dtype=torch.float32
        )
        self.curr_obs_torch["color_tensor"] = (
            torch.from_numpy(color).to(self.device, dtype=torch.float32) / 255.0
        )
    ...
    def compute_alignment(self, v_i, v_j, inst_n, inst_m):
        ...
        pcd_i_np = depth2fgpcd(
            depth_i.cpu().numpy(), mask_i_inst_n.cpu().numpy(), camera_param_i
        )
        ...
        pcd_i = torch.cat(
            [
                pcd_i.T,
                torch.ones(
                    (1, pcd_i.shape[0]), device=self.device, dtype=torch.float32
                ),
            ],
            dim=0,
        )
        ...
        pcd_j_np = depth2fgpcd(
            depth_j.cpu().numpy(), mask_j_inst_m.cpu().numpy(), camera_param_j
        )
        ...
        pcd_j = torch.cat(
            [
                pcd_j.T,
                torch.ones(
                    (1, pcd_j.shape[0]), device=self.device, dtype=torch.float32
                ),
            ],
            dim=0,
        )
        ...
        # project i in j
        pts_2d_i_in_j, valid_mask, pts_depth_i_in_j = project_points_coords(
            pcd_i, pose_j[None], K_j[None]
        )  # [1,N,2], [1,N], [1,N,1]
        pts_depth_i_in_j = pts_depth_i_in_j[..., 0]  # [1,N]
        inter_depth_i_in_j = interpolate_feats(
            depth_j[None, None],
            pts_2d_i_in_j,
            h=self.H,
            w=self.W,
        ...
        inter_mask_i_in_j = interpolate_feats(
            mask_j_inst_m[None, None].to(torch.float32),
            pts_2d_i_in_j,
            h=self.H,
            w=self.W,
            padding_mode="zeros",
        ...
        # project j in i
        pts_2d_j_in_i, valid_mask, pts_depth_j_in_i = project_points_coords(
            pcd_j, pose_i[None], K_i[None]
        )  # [1,N,2], [1,N], [1,N,1]
        pts_depth_j_in_i = pts_depth_j_in_i[..., 0]  # [1,N]
        inter_depth_j_in_i = interpolate_feats(
            depth_i[None, None],
            pts_2d_j_in_i,
            h=self.H,
            w=self.W,
        ...
        inter_mask_j_in_i = interpolate_feats(
            mask_i_inst_n[None, None].to(torch.float32),
            pts_2d_j_in_i,
            h=self.H,
            w=self.W,
            padding_mode="zeros",
        ...
        num_vis = (
            (torch.abs(pts_depth_i_in_j - inter_depth_i_in_j) < 0.03)
            & inter_mask_i_in_j
        ).sum().item() + (
            (torch.abs(pts_depth_j_in_i - inter_depth_j_in_i) < 0.03)
            & inter_mask_j_in_i
        ).sum().item()
    ...
    def add_mask_in_j_use_i_inst_n(self, v_i, v_j, inst_n):
        ...
        mask_i_inst_n = (
            cv2.erode(mask_i_inst_n * 255, np.ones([10, 10], np.uint8), iterations=1)
            / 255
        ).astype(bool)
        ...
        pts_i_visible_in_j = (
            (torch.abs(pts_depth_i_in_j - inter_depth_i_in_j) < 0.02) & valid_mask
        )[
            0
        ]  # [N]
    ...
    def compute_alignment_with_consensus(
        self, v_i, inst_n, consensus_mask_dict, consensus_i
    ):
        ...
        for label_i, label in enumerate(mask_label_i):
            ...
            for consensus_i, consensus_label in enumerate(consensus_mask_label):
                if label != consensus_label:
                    ...
                    continue
                align_ls.append(
                    self.compute_alignment_with_consensus(
                        v_i, label_i, consensus_mask_dict, consensus_i
                    )
                )
            ...
            try:
                ...
                print(
                    f"{v_i} is already in {consensus_mask_dict} at {matched_consensus_i}"
                )
    ...
    def update_with_additional_label(
        self, consensus_mask_label, consensus_mask_dict, v_i
    ):
        len_i = len(self.curr_obs_torch["mask_label"][v_i])
        mask_label_i = self.curr_obs_torch["mask_label"][v_i]
        len_curr = len(consensus_mask_label)
    ...
    def adjust_masks_using_consensus(
        self, consensus_mask_label, consensus_mask_dict, use_other
    ):
            ...
            for v_i, label_i in consensus_mask_dict[consensus_i].items():
                if label_i == -1:
                    ...
                    for v_j in visible_views:
                        ...
                        if add_mask is None:
                            continue
                        consensus_mask_dict[consensus_i][v_i] = len(
                            self.curr_obs_torch["mask_label"][v_i]
                        )
    ...
    def order_consensus(self, consensus_mask_label, consensus_mask_dict, queries):
        queries_wo_period = [
            query[:-1] if query[-1] == "." else query for query in queries
        ]
        ...
        for query in queries_wo_period:
            ...
            for i, label in enumerate(consensus_mask_label):
                if label == query:
                    ...
                    new_consesus_mask_dict[
                        len(new_consesus_mask_label) - 1
                    ] = consensus_mask_dict[i]
    ...
    def align_instance_mask_v2(self, queries, use_other=True):
        ...
        consensus_mask_dict = (
            {}
        )  # map label idx to a dict, each dict contains the view index and the instance id
        for i, label in enumerate(consensus_mask_label):
        ...
        for i in range(1, num_view):
            ...
            if len_i < len_curr:
                ...
                (
                    ...
                    consensus_mask_dict,
                ) = self.update_with_missing_label(
                    consensus_mask_label, consensus_mask_dict, i
                )
            elif len_i == len_curr:
                ...
                (
                    consensus_mask_label,
                    consensus_mask_dict,
                ) = self.update_with_additional_label(
                    consensus_mask_label, consensus_mask_dict, i
                )
                consensus_mask_label, consensus_mask_dict = self.order_consensus(
                    consensus_mask_label, consensus_mask_dict, queries
                )

        self.adjust_masks_using_consensus(
            consensus_mask_label, consensus_mask_dict, use_other=use_other
        )
    ...
    # TODO call 2: fusion.text_queries_for_inst_mask_no_track(query_texts, query_thresholds)
    def text_queries_for_inst_mask_no_track(self, queries, thresholds, merge_all=False):
        ...
        labels = []
        for i in range(self.num_cam):
            mask, label = grounded_instance_sam_bacth_queries_np(
    ...
    def text_queries_for_inst_mask(
        self, queries, thresholds, use_sam=False, merge_all=False
    ):
        ...
        if not self.xmem_first_mask_loaded:
            ...
            query_mask = instance2onehot(
                query_mask, len(self.track_ids)
            )  # [num_cam, H, W, num_instance]
            query_mask = self.align_with_prev_mask(
                query_mask
            )  # [num_cam, H, W, num_instance]
            query_mask = onehot2instance(query_mask)  # [num_cam, H, W]
            self.curr_obs_torch["mask"] = self.xmem_process(
                self.curr_obs_torch["color"], query_mask
            )  # [num_cam, H, W, num_instance]
        ...
        def drawHeatmap(event, x, y, flags, param):
            if event == cv2.EVENT_MOUSEMOVE:
                ...
                tgt_feat_sims_tensor = compute_similarity_tensor(
                    res["dino_feats"], src_feat_tensor, scale=sim_scale, dist_type="l2"
                )  # [N]
                tgt_feat_sims_tensor = (
                    tgt_feat_sims_tensor - tgt_feat_sims_tensor.min()
                ) / (
                    tgt_feat_sims_tensor.max() - tgt_feat_sims_tensor.min()
                )  # [N]
                tgt_feat_sims = tgt_feat_sims_tensor.detach().cpu().numpy()
                tgt_feat_sim_imgs = (viridis_cmap(tgt_feat_sims)[..., :3] * 255)[
                    ..., ::-1
                ]

                pts_2d, _, _ = project_points_coords(
                    pts, tgt_info["pose"], tgt_info["K"]
                )  # [N, 2]
                ...
                match_pt_2d, _, _ = project_points_coords(
                    match_pt_3d, tgt_info["pose"], tgt_info["K"]
                )
                ...
                for idx in range(num_tgt):
                    ...
                    pts_2d_flat_idx = np.ravel_multi_index(
                        pts_2d_i[valid_pts].T, (self.H, self.W)
                    )
                    ...
                    tgt_imshow_curr = draw_keypoints(
                        tgt_info["color"][idx],
                        match_pt_2d[idx],
                        colors=[(255, 0, 0)],
                        radius=5,
                    ...
                    merge_img = np.concatenate(
                        [heatmap, tgt_imshow_curr[..., ::-1]], axis=1
                    )
                    merge_imgs.append(merge_img)

                merge_imgs = np.concatenate(merge_imgs, axis=0)
                merge_imgs = cv2.resize(
                    merge_imgs,
                    (
                        int(merge_imgs.shape[1] * imshow_scale),
                        int(merge_imgs.shape[0] * imshow_scale),
                    ),
    ...
    def interactive_corr_img(self, src_info, tgt_info):
        ...
        def drawHeatmap(event, x, y, flags, param):
            ...
            if event == cv2.EVENT_MOUSEMOVE:
                ...
                tgt_feat_sim = (
                    compute_similarity_tensor(
                        tgt_dino, src_feat, scale=0.5, dist_type="l2"
                    )[0]
                    .detach()
                    .cpu()
                    .numpy()
                )
                tgt_feat_sim = (tgt_feat_sim - tgt_feat_sim.min()) / (
                    tgt_feat_sim.max() - tgt_feat_sim.min()
                )
                tgt_feat_sim_img = (viridis_cmap(tgt_feat_sim)[..., :3] * 255)[
                    ..., ::-1
                ]
                # tgt_feat_sim_img = cv2.resize(tgt_feat_sim_img, (tgt_img.shape[1], tgt_img.shape[0]), interpolation=cv2.INTER_NEAREST)
                tgt_color_render_curr = (
                    tgt_feat_sim_img * tgt_feat_sim[..., None]
                    + tgt_img * (1.0 - tgt_feat_sim[..., None])
                ).astype(np.uint8)
                cv2.imshow("tgt", tgt_color_render_curr)
                cv2.imshow("heatmap", tgt_feat_sim_img.astype(np.uint8))
    ...
    # TODO call 4: vertices, triangles = fusion.extract_mesh(init_grid, out, grid_shape)
    ...
    def create_mask_mesh(self, vertices, triangles, res):
        ...
        for i in range(query_masks.shape[1]):
            vertices_color = trimesh.visual.interpolate(
                query_masks[:, i], color_map="viridis"
            )
            mask_meshes.append(
                trimesh.Trimesh(
                    vertices=vertices,
    ...
    def create_instance_mask_mesh(self, vertices, triangles, res):
        ...
        for k in res.keys():
            if k.startswith("mask"):
                ...
                vertices_color = trimesh.visual.interpolate(
                    mask / num_instance, color_map="jet"
                )
                mask_meshes.append(
                    trimesh.Trimesh(
                        vertices=vertices,
    ...
    def create_descriptor_mesh(self, vertices, triangles, res, params, mask_out_bg):
        ...
        features_rgb = np.concatenate(
            [features_rgb, np.ones((features_rgb.shape[0], 1), dtype=np.uint8) * 255],
            axis=1,
        )
        features_mesh = trimesh.Trimesh(
            vertices=vertices, faces=triangles[..., ::-1], vertex_colors=features_rgb
        )
    ...
    def select_features(self, color, params):
        ...
        def select_kypts(event, x, y, flags, param):
            if event == cv2.EVENT_LBUTTONDBLCLK:
                ...
                color_curr = draw_keypoints(
                    color, np.array([[x, y]]), colors=[(0, 0, 255)], radius=5
                )
                cv2.imshow("src", color_curr[..., ::-1])
            elif event == cv2.EVENT_MOUSEMOVE:
                color_curr = draw_keypoints(
                    color, np.array([[x, y]]), colors=[(255, 0, 0)], radius=5
                )
                for kp in param["src_pts"]:
                    color_curr = draw_keypoints(
                        color_curr, np.array([kp]), colors=[(0, 0, 255)], radius=5
                    )
                cv2.imshow("src", color_curr[..., ::-1])
        ...
        color_with_kypts = draw_keypoints(
            color_with_kypts,
    ...
    def compute_conf(self, matched_pts, tgt_feats, conf_sigma):
        ...
        feat_dist = torch.norm(
            matched_pts_eval["dino_feats"] - tgt_feats, dim=1
        )  # (K, )
        conf = (
            torch.exp(-feat_dist / conf_sigma)
            * torch.exp(-torch.abs(matched_pts_eval["dist"]) * 50)
            * matched_pts_eval["valid_mask"]
        )  # (K, )
        return conf

    def find_correspondences_with_mask(
        self, src_feat_info, pts, last_match_pts_list, res, debug=False, debug_info=None
    ):
        ...
        for i in range(1, num_instances):
            src_feats = src_feat_info[self.curr_obs_torch["mask_label"][0][i]][
                "src_feats"
            ]
            if last_match_pts_list is None:
                last_match_pts = None
            else:
                last_match_pts = torch.from_numpy(last_match_pts_list[i - 1]).to(
                    self.device, dtype=torch.float32
                )
            ...
            sim_tensor = compute_similarity_tensor_multi(
                tgt_feats, src_feats, pts_i, last_match_pts, scale=0.5, dist_type="l2"
            )  # (N', K)
            # sim_tensor = sim_tensor * torch.clamp_min(torch.log(instance_mask[:, None] + 1e-7) + 1, 0) # (N', K)
            match_pts = extract_kypts_gpu(
                sim_tensor, pts_i, match_metric="sum"
            )  # (K, 3)

            topk_sim_idx = torch.topk(sim_tensor, k=1000, dim=0)[1]  # (100, K)
            topk_pts = pts_i[topk_sim_idx]  # (100, K, 3)
            observability = self.compute_conf(
                match_pts, src_feats, conf_sigma=1000.0
            )  # (K, )
    ...
    def find_correspondences(
        ...
        for i in range(3):
            ...
            try:
                ...
                exit()
            tgt_feats = out["dino_v2_feats"][
                out["valid_mask"] & mask_instance
            ]  # (N', f)
            curr_valid_que_pts = curr_que_pts[
                out["valid_mask"] & mask_instance
            ]  # (N', 3)
            assert tgt_feats.shape[0] == curr_valid_que_pts.shape[0]
            sim_vol = compute_similarity_tensor_multi(
                tgt_feats, src_feats, None, None, scale=0.5, dist_type="l2"
            )  # (N', K)
            next_que_pts, next_res = octree_subsample(
                sim_vol, curr_valid_que_pts, curr_res, topK=200
            )
    ...
    def tracking(self, src_feat_info, last_match_pts_list, boundaries, rand_ptcl_num):
        # :param src_feat_info dict
        ...
        for i in range(3):
            ...
            for instance_id in range(1, mask.shape[1]):
                ...
                if last_match_pts_list is None:
                    ...
                    last_match_pts_i = torch.from_numpy(
                        last_match_pts_list[instance_id - 1]
                    ).to(
                        self.device, dtype=torch.float32
                    )  # (K, 3)
                sim_vol = compute_similarity_tensor_multi(
                ...
                next_que_pts, next_res = octree_subsample(
                    sim_vol, curr_valid_que_pts, curr_res, topK=1000
                )
                curr_que_pts_list.append(next_que_pts)
            curr_res = next_res
            del curr_que_pts
        ...
        # else:
        #     debug_info = None
        match_pts_list, conf_list = self.find_correspondences_with_mask(
            src_feat_info,
            curr_que_pts,
            last_match_pts_list,
            out,
        ...
        for semantic_label in self.curr_obs_torch["semantic_label"][1:]:
            instance_indices = find_indices(
                self.curr_obs_torch["mask_label"][0], semantic_label
            )
    ...
    def select_conf_pts(
        self, src_feat_info, avg_conf_per_semantic, database, output_dir
    ):
        avg_window = 5
        ...
        for semantic_i, conf in enumerate(avg_conf_per_semantic):
            src_pts = src_feat_info[
                self.curr_obs_torch["semantic_label"][semantic_i + 1]
            ]["src_pts"]
            ...
            for idx in sorted_idx:
                if (
                    len(topk_idx) == 0
                    or np.min(np.linalg.norm(src_pts[topk_idx] - src_pts[idx], axis=1))
                    > 0.05
                ):
                    topk_idx.append(idx)
        ...
        partial_vid = cv2.VideoWriter(
            partial_vid_path, cv2.VideoWriter_fourcc(*"mp4v"), 2, (640 * 2, 360 * 2)
        )
        topk_match_pts_over_time = []
        for sel_time in tqdm(database.get_times()[::5]):
            match_pts_3d = np.load(
                os.path.join(full_pts_path, f"{sel_time:06d}.npy")
            )  # [instance_num, rand_ptcl_num, 3]
            topk_match_pts_list = []
            for semantic_idx, semantic_label in enumerate(
                self.curr_obs_torch["semantic_label"][1:]
            ):
                instance_indices = find_indices(
                    self.curr_obs_torch["mask_label"][0], semantic_label
                )
            ...
            if sel_time < avg_window:
                ...
                topk_match_pts_over_time.append(
                    np.stack(topk_match_pts_list, axis=0)
                )  # list of [instance_num, topk, 3]
                topk_match_pts_over_time_np = np.stack(
                    topk_match_pts_over_time, axis=0
                )  # [avg_window, instance_num, topk, 3]
                topk_match_pts_list = topk_match_pts_over_time_np.mean(
                    axis=0
                )  # [instance_num, topk, 3]
            ...
            for view_i in range(4):
                ...
                w_idx = view_i % 2
                merge_img[
                    h_idx * 360 : (h_idx + 1) * 360, w_idx * 640 : (w_idx + 1) * 640
                ] = vis_tracking_multimodal_pts(
                    database,
                    topk_match_pts_list,
                    None,
                    sel_time,
                    self.curr_obs_torch["mask"].detach().cpu().numpy(),
                    view_idx=view_i,
                )
    ...
    def smooth_tracking(
        ...
        last_match_pts_np = np.concatenate(
        ...
        W = torch.zeros((num_instance * rand_ptcl_num, 3)).to(
            self.device, dtype=torch.float32
        )
        W.requires_grad_()
        K = torch.zeros(
            (num_instance * rand_ptcl_num, num_instance * rand_ptcl_num)
        ).to(self.device, dtype=torch.float32)
        for i in range(num_instance):
            ...
            self_dist = torch.norm(
                src_pts.unsqueeze(0) - src_pts.unsqueeze(1), dim=-1
            )  # [rand_ptcl_num, rand_ptcl_num]
            K[
                i * rand_ptcl_num : (i + 1) * rand_ptcl_num,
                i * rand_ptcl_num : (i + 1) * rand_ptcl_num,
