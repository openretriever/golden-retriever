...
class MolmoActor:
    def __init__(self, gpu_id: int):
        ...
        try:
            ...
            self.console.print(f"[yellow]Initializing Molmo Actor on GPU {gpu_id}...[/yellow]")
            ...
            self.console.print(f"[green]Successfully initialized Molmo Actor on GPU {gpu_id}[/green]")
            
            # Test the pipeline with a small dummy input to ensure it's working
            dummy_image = Image.new('RGB', (224, 224), color='white')
            dummy_prompt = "Point at the object"
            self.pipe([(dummy_prompt, dummy_image)])
            self.console.print(f"[green]Verified Molmo Actor on GPU {gpu_id} with test input[/green]")
            
        except Exception as e:
            self.console.print(f"[red]Error initializing Molmo Actor on GPU {gpu_id}: {str(e)}[/red]")
            raise
    ...
    def get_gpu_info(self):
        ...
        return f"Running on GPU {self.gpu_id} ({torch.cuda.get_device_name(self.gpu_id)})"
...
class MolmoSAM2Service:
    def __init__(self,
        ...
        # Initialize Molmo actors for each GPU with timeout
        self.molmo_actors = []
        for gpu_id in self.gpu_ids:
            try:
                ...
                try:
                    ray.get(actor.get_gpu_info.remote(), timeout=60)  # 60 second timeout
                    self.molmo_actors.append(actor)
                    if self.verbose:
                        console.print(f"[green]Successfully created and verified Molmo Actor on GPU {gpu_id}[/green]")
                except Exception as e:
