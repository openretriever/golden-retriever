...
class LangSAM:
    ...
    def save(self, masks, boxes, phrases, logits, image_pil):
        if len(masks) == 0:
            ...
            utils.display_image_with_masks(image_pil, masks_np)

            # Display the image with bounding boxes and confidence scores
            utils.display_image_with_boxes(image_pil, boxes, logits, phrases)

            # Save the masks
            for i, mask_np in enumerate(masks_np):
                mask_path = f"image_mask_{i + 1}.png"
                utils.save_mask(mask_np, mask_path)

            # Print the bounding boxes, phrases, and logits
            # utils.print_bounding_boxes(boxes)
            # utils.print_detected_phrases(phrases)
            # utils.print_logits(logits)

            labels = [
                f"{phrase} {logit:.2f}"
                for phrase, logit in zip(phrases, logits, strict=False)
            ]
            image_array = np.asarray(image_pil)
            # fix mask problem
            image = utils.draw_image(image_array, [], boxes, labels)
            image = Image.fromarray(np.uint8(image)).convert("RGB")
            # image.show()
            utils.save_image(image, filename="image_langsam.png")
