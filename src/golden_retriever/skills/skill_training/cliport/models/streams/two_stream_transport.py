import src.skill_training.cliport.models as models
import src.skill_training.cliport.models.core.fusion as fusion
...
class TwoStreamTransport(Transport):
    ...
    def __init__(
        ...
        super().__init__(
            stream_fcn, in_shape, n_rotations, crop_size, preprocess, cfg, device
        )
    ...
    def _build_nets(self):
        ...
        self.key_stream_one = stream_one_model(
            self.in_shape, self.output_dim, self.cfg, self.device, self.preprocess
        )
        self.key_stream_two = stream_two_model(
            self.in_shape, self.output_dim, self.cfg, self.device, self.preprocess
        )
        self.query_stream_one = stream_one_model(
            self.kernel_shape, self.kernel_dim, self.cfg, self.device, self.preprocess
        )
        self.query_stream_two = stream_two_model(
            self.in_shape, self.kernel_dim, self.cfg, self.device, self.preprocess
        )
    ...
    def transport(self, in_tensor, crop):
        logits = self.fusion_key(
            self.key_stream_one(in_tensor), self.key_stream_two(in_tensor)
        )
        kernel = self.fusion_query(
            self.query_stream_one(crop), self.query_stream_two(crop)
        )
