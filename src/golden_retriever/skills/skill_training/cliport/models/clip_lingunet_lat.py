...
from retriever.skill_training.cliport.models.core import fusion
from retriever.skill_training.cliport.models.core.clip import build_model, load_clip, tokenize
from retriever.skill_training.cliport.models.core.fusion import FusionConvLat
from retriever.skill_training.cliport.models.core.unet import Up
from retriever.skill_training.cliport.models.resnet import ConvBlock, IdentityBlock
...
class CLIPLingUNetLat(nn.Module):
    ...
    def _build_decoder(self):
        # language
        self.lang_fuser1 = fusion.names[self.lang_fusion_type](
            input_dim=self.input_dim // 2
        )
        self.lang_fuser2 = fusion.names[self.lang_fusion_type](
            input_dim=self.input_dim // 4
        )
        self.lang_fuser3 = fusion.names[self.lang_fusion_type](
            input_dim=self.input_dim // 8
        )
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(
                self.input_dim, 1024, kernel_size=3, stride=1, padding=1, bias=False
            ),
            nn.ReLU(True),
        )
        ...
        self.layer2 = nn.Sequential(
            ConvBlock(
                64, [32, 32, 32], kernel_size=3, stride=1, batchnorm=self.batchnorm
            ),
            IdentityBlock(
                32, [32, 32, 32], kernel_size=3, stride=1, batchnorm=self.batchnorm
            ),
            nn.UpsamplingBilinear2d(scale_factor=2),
        )
        ...
        self.layer3 = nn.Sequential(
            ConvBlock(
                32, [16, 16, 16], kernel_size=3, stride=1, batchnorm=self.batchnorm
            ),
            IdentityBlock(
                16, [16, 16, 16], kernel_size=3, stride=1, batchnorm=self.batchnorm
            ),
            nn.UpsamplingBilinear2d(scale_factor=2),
        )
