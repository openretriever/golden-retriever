...
from retriever.envs.ravens import dataset, tasks
from retriever.envs.ravens.envs.environment import Environment
from retriever.skill_training.cliport import agents
        ...
        for train_run in range(vcfg["n_repeats"]):
            ...
            for i in range(0, n_demos):
                ...
                for _ in range(task.max_steps):
                    ...
                    print(
                        f"Total Reward: {total_reward:.3f} | Done: {done} | Success: {success}\n"
                    )
                    if done or success:
                        break
...
def list_ckpts_to_eval(vcfg, existing_results):
    ...
    # Validation checkpoints that haven't been already evaluated.
        ...
        ckpts_to_eval = [c for _, c in sorted(zip(steps, ckpts_to_eval, strict=False))]
