...
from retriever.envs.ravens.dataset import RavensDataset, RavensMultiTaskDataset
from retriever.skills.cliport import agents
...
def main(cfg):
    # Logger
    # np.random.seed(cfg['lepp']['seed'])  # make sure the training data is same among methods
    # Checkpoint saver
    ...
    # if 'real' in dataset_type:
    # train_ds = RealDataset(os.path.join(data_dir, '{}.npy'.format(task)), cfg, n_demos=n_demos, augment=True)
    #     else:
