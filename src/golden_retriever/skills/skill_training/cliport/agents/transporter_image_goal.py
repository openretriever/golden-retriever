...
from retriever.skill_training.cliport.agents.transporter import OriginalTransporterAgent
from retriever.skill_training.cliport.models.core.attention import Attention
from retriever.skill_training.cliport.models.core.attention_image_goal import (
    AttentionImageGoal,
)
from retriever.skill_training.cliport.models.core.transport_image_goal import (
    TransportImageGoal,
)
...
class ImageGoalTransporterAgent(OriginalTransporterAgent):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i in range(self.val_repeats):
            frame, goal = batch
            l0, err0 = self.attn_training_step(
                frame, goal, backprop=False, compute_err=True
            )
            loss0 += l0
            if isinstance(self.transport, Attention):
