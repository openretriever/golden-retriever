...
from retriever.skill_training.cliport.models.core.attention import Attention
from retriever.skill_training.cliport.models.core.transport import Transport
from retriever.skill_training.cliport.models.streams.two_stream_attention import (
    TwoStreamAttention,
    TwoStreamAttentionLat,
)
from retriever.skill_training.cliport.models.streams.two_stream_transport import (
    TwoStreamTransport,
    TwoStreamTransportLat,
)
...
class TransporterAgent(LightningModule):
    def __init__(self, name, cfg, train_ds, test_ds):
        ...
        self._build_model()
        self._optimizers = {
            "attn": torch.optim.Adam(
                self.attention.parameters(), lr=self.cfg["train"]["lr"]
            ),
            "trans": torch.optim.Adam(
                self.transport.parameters(), lr=self.cfg["train"]["lr"]
            ),
        }
    ...
    def transport_training_step(self, frame, backprop=True, compute_err=False):
        ...
        err, loss = self.transport_criterion(
            backprop, compute_err, inp, output, p0, p1, p1_theta
        )
        return loss, err
    ...
    def check_save_iteration(self):
        ...
        if (global_step + 1) in self.save_steps:
            ...
            checkpoint_path = os.path.join(
                self.cfg["train"]["train_dir"], "checkpoints"
            )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i in range(self.val_repeats):
            ...
            loss0 += l0
            if isinstance(self.transport, Attention):
                l1, err1 = self.attn_training_step(
                    frame, backprop=False, compute_err=True
                )
                loss1 += l1
            else:
                l1, err1 = self.transport_training_step(
                    frame, backprop=False, compute_err=True
                )
                loss1 += l1
        ...
        self.trainer.evaluation_loop.trainer.train_loop.running_loss.append(
            val_total_loss
        )
    ...
    def validation_epoch_end(self, all_outputs):
        ...
        print(
            "\nAttn Err - Dist: {:.2f}, Theta: {:.2f}".format(
                total_attn_dist_err, total_attn_theta_err
            )
        )
        print(
            "Transport Err - Dist: {:.2f}, Theta: {:.2f}".format(
                total_trans_dist_err, total_trans_theta_err
            )
        )
