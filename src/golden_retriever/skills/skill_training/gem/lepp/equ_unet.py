...
class EquResUNet(torch.nn.Module):
    def __init__(
                ...
                [
                    (
                        ...
                        nn.R2Conv(
                            nn.FieldType(
                                self.r2_act,
                                n_input_channel * [self.r2_act.trivial_repr],
                            ),
            ...
            OrderedDict(
                [
                    (
                        "enc-pool-2",
                        nn.PointwiseMaxPool(
                            nn.FieldType(self.r2_act, self.l1_c * [self.repr]), 2
                        ),
                    ),
                    (
                        "enc-e2res-2",
            ...
            OrderedDict(
                [
                    (
                        "enc-pool-3",
                        nn.PointwiseMaxPool(
                            nn.FieldType(self.r2_act, self.l2_c * [self.repr]), 2
                        ),
                    ),
                    (
                        "enc-e2res-3",
            ...
            OrderedDict(
                [
                    (
                        "enc-pool-4",
                        nn.PointwiseMaxPool(
                            nn.FieldType(self.r2_act, self.l3_c * [self.repr]), 2
                        ),
                    ),
                    (
                        "enc-e2res-4",
            ...
            OrderedDict(
                [
                    (
                        "enc-pool-5",
                        nn.PointwiseMaxPool(
                            nn.FieldType(self.r2_act, self.l4_c * [self.repr]), 2
                        ),
                    ),
                    (
                        "enc-e2res-5",
        ...
        self.upsample_16_8 = nn.R2Upsampling(
            nn.FieldType(self.r2_act, self.l4_c * [self.repr]), 2
        )
        self.upsample_8_4 = nn.R2Upsampling(
            nn.FieldType(self.r2_act, self.l3_c * [self.repr]), 2
        )
        self.upsample_4_2 = nn.R2Upsampling(
            nn.FieldType(self.r2_act, self.l2_c * [self.repr]), 2
        )
        self.upsample_2_1 = nn.R2Upsampling(
            nn.FieldType(self.r2_act, self.l1_c * [self.repr]), 2
        )

    def forwardEncoder(self, obs):
        obs_gt = nn.GeometricTensor(
            obs, nn.FieldType(self.r2_act, obs.shape[1] * [self.r2_act.trivial_repr])
        )
    ...
    def forwardDecoder(
        ...
        concat_8 = torch.cat(
            (feature_map_8.tensor, self.upsample_16_8(feature_map_16).tensor), dim=1
        )
        concat_8 = nn.GeometricTensor(
            concat_8, nn.FieldType(self.r2_act, 2 * self.l4_c * [self.repr])
        )
        feature_map_up_8 = self.conv_up_8(concat_8)

        concat_4 = torch.cat(
            (feature_map_4.tensor, self.upsample_8_4(feature_map_up_8).tensor), dim=1
        )
        concat_4 = nn.GeometricTensor(
            concat_4, nn.FieldType(self.r2_act, 2 * self.l3_c * [self.repr])
        )
        feature_map_up_4 = self.conv_up_4(concat_4)

        concat_2 = torch.cat(
            (feature_map_2.tensor, self.upsample_4_2(feature_map_up_4).tensor), dim=1
        )
        concat_2 = nn.GeometricTensor(
            concat_2, nn.FieldType(self.r2_act, 2 * self.l2_c * [self.repr])
        )
        feature_map_up_2 = self.conv_up_2(concat_2)

        concat_1 = torch.cat(
            (feature_map_1.tensor, self.upsample_2_1(feature_map_up_2).tensor), dim=1
        )
        concat_1 = nn.GeometricTensor(
            concat_1, nn.FieldType(self.r2_act, 2 * self.l1_c * [self.repr])
        )
    ...
    def forward(self, obs):
        (
            ...
            feature_map_8,
            feature_map_16,
        ) = self.forwardEncoder(obs)
        return self.forwardDecoder(
...
class EquiUnet(torch.nn.Module):
    def __init__(self, in_dim, out_dim, N=6, middle_dim=(32, 64, 128, 256), init=False):
        ...
        self.r2_act = gspaces.Rot2dOnR2(N=N)
        self.main_block = EquResUNet(
            n_input_channel=in_dim,
        ...
        self.final = torch.nn.Sequential(
            nn.R2Conv(
                nn.FieldType(self.r2_act, [self.r2_act.regular_repr] * middle_dim[0]),
...
class EquiUnetCLIPPostLinearADD(torch.nn.Module):
    def __init__(
        self,
        preprocess,
        in_dim,
        out_dim,
        N=6,
        middle_dim=(32, 64, 128, 256),
        init=False,
    ):
        super(EquiUnetCLIPPostLinearADD, self).__init__()
        ...
        shape_out = 3
        self.conv_clip = torch.nn.Sequential(
            nn.R2Conv(
                nn.FieldType(self.r2_act, (1 + shape_out) * [self.r2_act.trivial_repr]),
                nn.FieldType(self.r2_act, [self.r2_act.trivial_repr] * out_dim),
                kernel_size=9,
                padding=4,
                initialize=False,
        ...
        self.shape_conv = nn.SequentialModule(
            nn.R2Conv(
                nn.FieldType(self.r2_act, [self.r2_act.trivial_repr]),
                nn.FieldType(self.r2_act, 3 * [self.r2_act.regular_repr]),
                kernel_size=3,
                padding=1,
                bias=False,
            ),
            nn.ReLU(
                nn.FieldType(self.r2_act, 3 * [self.r2_act.regular_repr]), inplace=True
            ),
            nn.R2Conv(
                nn.FieldType(self.r2_act, 3 * [self.r2_act.regular_repr]),
                nn.FieldType(self.r2_act, 3 * [self.r2_act.trivial_repr]),
                kernel_size=3,
                padding=1,
                bias=False,
            ),
    ...
    def forward(self, x, clip_feature, lan_emb):
        ...
        depth = nn.GeometricTensor(
            depth,
            nn.FieldType(self.r2_act, depth.shape[1] * [self.r2_act.trivial_repr]),
        )
        clip_feature = nn.GeometricTensor(
            clip_feature,
            nn.FieldType(
                self.r2_act, clip_feature.shape[1] * [self.r2_act.trivial_repr]
            ),
        )
        shape_feature = self.shape_conv(depth)
        clip_feature = torch.cat(
            [clip_feature.tensor, shape_feature.tensor], axis=1
