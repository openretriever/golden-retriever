...
class StackBlockPyramidSeqUnseenColors(Task):
    ...
    def reset(self, env):
        ...
        self.goals.append(
            ([objs[0]], np.ones((1, 1)), [targs[0]], False, True, "pose", None, 1 / 6)
        )
        self.lang_goals.append(
            self.lang_template.format(
                pick=color_names[0], place="the lightest brown block"
            )
        )

        self.goals.append(
            ([objs[1]], np.ones((1, 1)), [targs[1]], False, True, "pose", None, 1 / 6)
        )
        self.lang_goals.append(
            self.lang_template.format(
                pick=color_names[1], place="the middle brown block"
            )
        )

        self.goals.append(
            ([objs[2]], np.ones((1, 1)), [targs[2]], False, True, "pose", None, 1 / 6)
        )
        self.lang_goals.append(
            self.lang_template.format(
                pick=color_names[2], place="the darkest brown block"
            )
        )

        # Goal: make middle row.
        self.goals.append(
            ([objs[3]], np.ones((1, 1)), [targs[3]], False, True, "pose", None, 1 / 6)
        )
        self.lang_goals.append(
            self.lang_template.format(
                pick=color_names[3],
        ...
        self.goals.append(
            ([objs[4]], np.ones((1, 1)), [targs[4]], False, True, "pose", None, 1 / 6)
        )
        self.lang_goals.append(
            self.lang_template.format(
                pick=color_names[4],
