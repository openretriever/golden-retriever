...
def reconstruct_heightmaps(color, depth, configs, bounds, pixel_size):
    ...
    for color, depth, config in zip(color, depth, configs, strict=False):
...
def preprocess(img, dist="transporter"):
    ...
    real_color_mean = [0.643160, 0.64442464, 0.62976266]
    real_color_std = [0.0055112, 0.0052110, 0.0056037]
    real_depth_mean = 0.027036077
    real_depth_std = 0.000584195

    realtable_color_mean = [
        158.77528381347656 / 255.0,
        134.8521270751953 / 255.0,
        113.92375946044922 / 255.0,
    ]
    realtable_color_std = [
        51.23906707763672 / 255.0,
        47.6989860534668 / 255.0,
        47.66556930541992 / 255.0,
    ]
    realtable_depth_mean = 0.025506172329187393
    realtable_depth_std = 0.009955680929124355

    realspot_color_mean = [
        161.60982585781827 / 255.0,
        160.48708384720095 / 255.0,
        156.82210651241547 / 255.0,
    ]
    realspot_color_std = [
        44.7768041050561 / 255.0,
        48.056533762922314 / 255.0,
        52.61129027597777 / 255.0,
    ]
    realspot_depth_mean = 0.04385168810968904
    realspot_depth_std = 0.03249186446712478
    ...
    if dist == "clip":
        ...
        color_mean = real_color_mean
        color_std = real_color_std
    elif dist == "realtable":
        color_mean = realtable_color_mean
        color_std = realtable_color_std
    elif dist == "realspot":
        color_mean = realspot_color_mean
        color_std = realspot_color_std
    else:
    ...
    if dist == "franka":
        ...
        depth_mean = real_depth_mean
        depth_std = real_depth_std
    elif dist == "realtable":
        depth_mean = realtable_depth_mean
        depth_std = realtable_depth_std
    elif dist == "realspot":
        depth_mean = realspot_depth_mean
        depth_std = realspot_depth_std
    else:
...
def get_clip_feature_image(obs, clip_feature, configs, bounds, pix_size):
    ...
    cmap, hmap = get_fused_heightmap(obs, configs, bounds, pix_size)
    img = cmap
    # assert img.shape == (in_shape[0], in_shape[1], clip_feature.shape[3])
...
class ImageRotator:
    ...
    def __call__(self, x_list, pivot, reverse=False):
        ...
        for i, angle in enumerate(self.angles):
            ...
            M: torch.tensor = kornia.geometry.transform.imgwarp.get_rotation_matrix2d(
                center, angle, scale
            )

            # apply the transformation to original image
            _, _, h, w = x.shape
            x_warped: torch.tensor = kornia.geometry.transform.imgwarp.warp_affine(
