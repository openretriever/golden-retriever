...
class RavensDataset(Dataset):
    ...
    def __init__(self, path, cfg, n_demos=0, augment=False, subtask=None):
        """A simple RGB-D image dataset."""
        self._path = path
        self.root_path = "/" + os.path.join(*path.split("/")[:-1])
        ...
        self.topdown = self.cfg["dataset"]["topdown"]
    ...
    def load(self, episode_id, images=True, cache=False):
        ...
        for fname in sorted(os.listdir(path)):
            if f"{episode_id:06d}" in fname:
                ...
                if self.clip_type == "normal":
                    self.clip_type = ""

                clip_topdown_crop_feature = [None] * len(action)

                if self.use_image_goal and self.topdown:
                    clip_feature = load_field(
                        episode_id, "clip_topdown_crop" + self.clip_type, fname
                    )
                    assert clip_feature.dtype == np.uint8
                    clip_feature = (clip_feature / 255).astype(float)
                elif self.use_image_goal and not self.topdown:
                    clip_topdown_crop_feature = load_field(
                        episode_id, "clip_topdown_crop" + self.clip_type, fname
                    )
                    pp_feature_crop = load_field(
                        episode_id, "clip" + self.clip_type, fname
                    )
                    pick_feature_crop = load_field(
                        episode_id, "clip_pick" + self.clip_type, fname
                    )
                    place_feature_crop = load_field(
                        episode_id, "clip_place" + self.clip_type, fname
                    )

                    assert pp_feature_crop.dtype == np.uint8
                    clip_feature = np.concatenate(
                        [pp_feature_crop, pick_feature_crop, place_feature_crop],
                        axis=-1,
                    )
                    clip_feature = (clip_feature / 255).astype(float)
                    clip_topdown_crop_feature = (
                        clip_topdown_crop_feature / 255
                    ).astype(float)

                elif not self.use_image_goal and self.topdown:
                    clip_feature = load_field(
                        episode_id, "clip_topdown" + self.clip_type, fname
                    )
                    assert clip_feature.dtype == np.uint8
                    clip_feature = (clip_feature / 255).astype(float)
                else:
                    pp_feature = load_field(episode_id, "clip" + self.clip_type, fname)
                    pick_feature = load_field(
                        episode_id, "clip_pick" + self.clip_type, fname
                    )
                    place_feature = load_field(
                        episode_id, "clip_place" + self.clip_type, fname
                    )
                    assert pp_feature.dtype == np.uint8
                    clip_feature = np.concatenate(
                        [pp_feature, pick_feature, place_feature], axis=-1
                    )
                    clip_feature = (clip_feature / 255).astype(float)
                ...
                for i in range(len(action)):
                    ...
                    episode.append(
                        (
                            ...
                            info[i],
                            clip_feature[i],
                            clip_topdown_crop_feature[i],
                        )
                    )
                return episode, seed

    def valload(self, episode_id, images=True, cache=False):
        def load_field(episode_id, field, fname):
            # Check if sample is in cache.
            if cache:
                if episode_id in self._cache:
    ...
    def get_clip_feature_image(self, obs, clip_feature, cam_config=None):
        ...
        # Get color and height maps from RGB-D images.
        obs["color"] = (clip_feature * 255).astype(
            np.uint8
        )  # convert 0-1 score to 0-255 just for reconstruction
        cmap, hmap = utils.get_fused_heightmap(
            obs, cam_config, self.bounds, self.pix_size
        )
        img = cmap
        assert img.shape == (self.in_shape[0], self.in_shape[1], clip_feature.shape[3])
        return img.astype(float) / 255

    def process_sample(self, datum, augment=True):
        # Get training labels from data sample.
        (obs, act, _, info, clip_feature, clip_topdown_crop_feature) = datum
        img = self.get_image(obs)
        if self.topdown:
            clip_feature_img = clip_feature
        else:
            clip_feature_img = self.get_clip_feature_image(
                obs, clip_feature
            )  # channel0 is pp feature, channel1 is pick, channel2 is place.
            if self.use_image_goal:
                clip_feature_img = (
                    clip_feature_img * (1 - self.image_text_ratio)
                    + clip_topdown_crop_feature * self.image_text_ratio
                )
        ...
        if augment:
            ...
            feature_image = utils.apply_perturbation(clip_feature_img, perturb_params)
    ...
    def process_goal(self, goal, perturb_params):
        # Get goal sample.
        (obs, act, _, info, clip_feature, clip_topdown_crop_feature) = goal
        img = self.get_image(obs)
        if self.topdown:
            clip_feature_img = clip_feature
        else:
...
MULTI_TASKS = {  # goal-conditioned tasks
...
class RavensMultiTaskDataset(RavensDataset):
    ...
    def __init__(
        self, path, cfg, group="multi-all", mode="train", n_demos=100, augment=False
    ):
        ...
        self.attr_train_task = (
            self.MULTI_TASKS[group]["attr_train_task"]
            if "attr_train_task" in self.MULTI_TASKS[group]
            else None
        )
        ...
        for task in self.tasks:
            ...
            episodes[task] = np.random.choice(
                range(n_episodes), min(self.n_demos, n_episodes), False
            )
    ...
    def load(self, episode_id, images=True, cache=False):
        if self.attr_train_task is None or self.mode in ["val", "test"]:
            ...
            all_tasks = [
                self.attr_train_task
            ] + all_other_tasks  # add seen task in the front
            ...
            sampling_probs = [
                (1 - mult_attr_seen_sample_prob) / (len(all_tasks) - 1)
            ] * len(all_tasks)
    ...
    def valload(self, episode_id, images=True, cache=False):
        ...
        self._path = os.path.join(self.root_path, f"{self._task}-{self.mode}")
        return super().valload(episode_id, images, cache)
