...
from cliport.models.streams.one_stream_attention_lang_fusion import (
    OneStreamAttentionLangFusion,
)
from cliport.models.streams.one_stream_transport_lang_fusion import (
...
class TwoStreamClipLingUNetTransporterAgent(TransporterAgent):
    ...
    def attn_training_step(self, frame, backprop=True, compute_err=False):
        ...
        if self.parse:
            self.subtask = self.train_ds.get_curr_task()
            lang_goal, _ = parse_instruction(self.task, lang_goal, self.subtask)
    ...
    def act(self, obs, info, goal=None):  # pylint: disable=unused-argument
        ...
        pick_goal, place_goal = parse_instruction(self.task, lang_goal, self.subtask)

        # Attention model forward pass.
        pick_inp = {"inp_img": img, "lang_goal": pick_goal}
        ...
        place_inp = {"inp_img": img, "p0": p0_pix, "lang_goal": place_goal}
    ...
    def actReal(self, obs, info, goal=None):  # pylint: disable=unused-argument
        """Run inference and return best action given visual observations."""
        # Get heightmap from RGB-D images.
        img = obs["img"]
        lang_goal = info["lang_goal"]
        pick_goal, place_goal = parse_instruction(self.task, lang_goal)
...
class TwoStreamClipFilmLingUNetLatTransporterAgent(
...
class TwoStreamUntrainedRN50BertLingUNetTransporterAgent(
    TwoStreamClipLingUNetTransporterAgent
):
    def __init__(self, name, cfg, train_ds, test_ds):
        super().__init__(name, cfg, train_ds, test_ds)
...
class TwoStreamRN50BertLingUNetLatTransporterAgent(
    TwoStreamClipLingUNetTransporterAgent
):
    def __init__(self, name, cfg, train_ds, test_ds):
        super().__init__(name, cfg, train_ds, test_ds)
