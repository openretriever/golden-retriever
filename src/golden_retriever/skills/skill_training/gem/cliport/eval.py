...
import numpy as np
import wandb
from cliport import agents, dataset, tasks
        ...
        for train_run in range(vcfg["n_repeats"]):
            ...
            for i in range(0, n_demos):
                ...
                for r, i in results:
                    ...
                    task_is_success.append(1.0 if r == 1 else 0.0)

                mean_reward = np.mean(mean_reward)
                pp_sr = np.mean(task_is_success)
            ...
            if vcfg["wandb"]["enable"]:
                wandb.log({"ckpt": ckpt, "mean_reward": mean_reward, "pp_sr": pp_sr})
...
def list_ckpts_to_eval(vcfg, existing_results):
    ...
    # Validation checkpoints that haven't been already evaluated.
        ...
        checkpoints = sorted(
            ...
            key=lambda x: re.findall(r"\d+", x)[0].zfill(5),
        )
