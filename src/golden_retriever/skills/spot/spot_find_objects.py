...
from predicators.spot_utils.utils import (
    ...
    get_collision_geoms_for_nav,
    get_relative_se2_from_se3,
    sample_random_nearby_point_to_move,
    spot_pose_to_geom2d,
)
...
def _find_objects_with_choreographed_moves(
    ...
    detections, artifacts = detect_objects(
        object_ids, rgbds, allowed_regions=allowed_regions
    )
...
def step_back_to_find_objects(
    ...
    moves = [
        ...
        (math_helpers.SE2Pose(0.0, 0.0, np.pi / 6), DEFAULT_HAND_LOOK_FLOOR_POSE),
        # Spin to the left and look at the floor.
        (math_helpers.SE2Pose(0.0, 0.0, -np.pi / 6), DEFAULT_HAND_LOOK_FLOOR_POSE),
    ]
    base_moves, hand_moves = zip(*moves, strict=False)
...
def find_objects(
    ...
    try:
        step_back_to_find_objects(
            robot, localizer, object_ids, allowed_regions=allowed_regions
        )
    except RuntimeError:
        prompt = (
            "Hit 'c' to have the robot try to find the object "
...
if __name__ == "__main__":
    ...
    def _run_manual_test(hostname: str) -> None:
