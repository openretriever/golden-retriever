...
from retriever.skills.spot.spot_hand_move import (
    move_hand_to_relative_pose,
    move_hand_to_relative_pose_with_velocity,
)
from retriever.skills.spot.spot_navigation import navigate_to_relative_pose
from retriever.skills.spot.spot_stow_arm import stow_arm


def sweep(
    robot: Robot,
    sweep_start_pose: math_helpers.SE3Pose,
    move_dx: float,
    move_dy: float,
    move_dz: float,
    duration: float,
    ...
    sweep_start_conformant = math_helpers.SE3Pose(
        x=sweep_start_pose.x,
        y=sweep_start_pose.y,
    ...
    relative_hand_move = math_helpers.SE3Pose(
        x=move_dx, y=move_dy, z=move_dz, rot=math_helpers.Quat()
    )
    sweep_end_pose = relative_hand_move * sweep_start_conformant
    move_hand_to_relative_pose_with_velocity(
        robot, sweep_start_conformant, sweep_end_pose, duration
    )
    # Stow arm.
    stow_arm(robot)
...
if __name__ == "__main__":
    ...
    from predicators.spot_utils.skills.spot_hand_move import close_gripper, open_gripper
    from predicators.spot_utils.skills.spot_navigation import go_home
    from predicators.spot_utils.spot_localization import SpotLocalizer
    from predicators.spot_utils.utils import (
        get_graph_nav_dir,
    ...
    def _run_manual_test() -> None:
        ...
        black_table_pose = math_helpers.SE3Pose(2.3, -2.0, -0.32, math_helpers.Quat())
        ...
        detections, _ = init_search_for_objects(
            robot, localizer, {train_toy_detection_id}
        )
        ...
        rel_pose = get_relative_se2_from_se3(
            robot_pose, train_toy_pose, pre_sweep_nav_distance, pre_sweep_nav_angle
        )
        ...
        upper_left_black_table_rel_pose = (
            robot_pose.inverse() * upper_left_black_table_pose
        )
        middle_bottom_black_table_rel_pose = (
            robot_pose.inverse() * middle_bottom_black_table_pose
        )
        start_dx = 0.175
        start_dy = 0.4
        # clamp the x and y poses to the top left of the table worst case.
        start_x = np.clip(
            middle_bottom_black_table_rel_pose.x,
            train_toy_rel_pose.x + start_dx,
            upper_left_black_table_rel_pose.x,
        )
        start_y = np.clip(
            middle_bottom_black_table_rel_pose.y,
        ...
        rot = pitch * yaw
        sweep_start_pose = math_helpers.SE3Pose(
            x=start_x, y=start_y, z=start_z, rot=rot
        )
