...
from retriever.skills.spot.spot_hand_move import move_hand_to_relative_pose, open_gripper


def place_at_relative_position(
    robot: Robot,
    body_to_position: math_helpers.Vec3,
    downward_angle: float = np.pi / 2.5,
) -> None:
    ...
    body_tform_goal = math_helpers.SE3Pose(
        x=body_to_position.x, y=body_to_position.y, z=body_to_position.z, rot=rot
    )
