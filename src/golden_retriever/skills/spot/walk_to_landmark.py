...
from retriever.mappers.utils_graphnav import graph_nav_util
...
from retriever.skills.spot.grasp_door import stand, initialize_robot, check_estop
import ray
from glob import glob
class GraphNavInterface(object):
    """GraphNav service command line interface."""

    def __init__(self, robot, upload_path, molmo=None, options=None, use_gps=False):
        self._robot = robot
    ...
    def navigate_to_seed_x_y_yaw(self, x, y , yaw):
        """Navigate to a pose in seed frame, using anchors."""
        # The following options are accepted for arguments: [x, y], [x, y, yaw], [x, y, z, yaw],
        # [x, y, z, qw, qx, qy, qz].
        # When a value for z is not specified, we use the current z height.
        ...
        seed_T_goal = SE3Pose(float(x), float(y), 0.0, Quat())
        ...
        seed_T_goal.rot = Quat.from_yaw(float(yaw))
        if not self.toggle_power(should_power_on=True):
            print('Failed to power on the robot, and cannot complete navigate to request.')
            return
...
def load_annotations(file_path):
    with open(file_path, "r") as file:
        return yaml.safe_load(file)["annotations"]

def init_graphnav(graph_nav_command_line):
    graph_nav_command_line.upload_graph_and_snapshots()
    graph_nav_command_line.set_initial_localization_fiducial()
    graph_nav_command_line.list_graph_waypoint_and_edge_ids()
    graph_nav_command_line.get_localization_state()
...
def main():
    ...
    while True:
        ...
        choice = input("\nEnter a location name (or 'exit' to quit): ").strip()
        if choice.lower() == "exit":
            break
        
        if choice in annotations:
            entry = annotations[choice]
    ...
    """
    graph_nav_command_line.run("5")
    # List waypoints
    graph_nav_command_line.run("4")
    # Localize with fiducial
    graph_nav_command_line.run("2")
    
    # Navigate to Room 101 : big starting area: #7 : ar
    graph_nav_command_line.run("6 " + room_101_empty_space[0])

    # Navigate to door inside 101
    graph_nav_command_line.run("6 " + room_101_door_1['inside'])
    # Open the door
    open_door(options, robot, molmo)

    # Hallway: between the 2 doors of 101: #191 : ng
    graph_nav_command_line.run("6 ng")

    # Navigate to door outside 120
    graph_nav_command_line.run("6 " + room_120_door_1['outside'])
    # Open the door
    open_door(options, robot, molmo)

    # Navigate to Room 101 : big starting area: #7 : ar
    graph_nav_command_line.run("6 " + room_120_empty_space[0])

    graph_nav_command_line.run("6 " + room_120_door_1['inside'])
    open_door(options, robot, molmo)

    graph_nav_command_line.run("6 " + room_101_door_2['outside'])
    open_door(options, robot, molmo)

    graph_nav_command_line.run("6 " + room_101_empty_space[1])
    """
