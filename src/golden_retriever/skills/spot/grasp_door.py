...
def walk_to_object_in_image(robot, image, point, debug):
    ...
    manipulation_cmd.pixel_xy.x = point[0]
    manipulation_cmd.pixel_xy.y = point[1]
...
class RequestManager:
    ...
    def set_hinge_position_side_by_side(self, hinge_position_side_by_side):
        self.hinge_position_side_by_side = hinge_position_side_by_side

    def set_handle_position_side_by_side(self, handle_position_side_by_side):
        self.handle_position_side_by_side = handle_position_side_by_side
    ...
    def get_walk_to_object_in_image_request(self):
        ...
        if len(self.side_by_side.shape) == 3:
...
def vision_tform_sensor_func(image):
...
def open_door(robot, image, snapshot, hinge):
    ...
    vision_tform_sensor = vision_tform_sensor_func(image)
    ...
    #auto_cmd = door_pb2.DoorCommand.AutoPushCommand()
    ...
    auto_cmd.search_ray_start_in_frame.CopyFrom(geometry_pb2.Vec3(x=search_ray_start_in_frame[0], y=search_ray_start_in_frame[1], z=search_ray_start_in_frame[2]))
    #auto_cmd.push_point_in_frame.CopyFrom(geometry_pb2.Vec3(x=raycast_point_wrt_vision[0], y=raycast_point_wrt_vision[1], z=raycast_point_wrt_vision[2]))


    search_ray_end_in_frame = raycast_point_wrt_vision + search_ray
    auto_cmd.search_ray_end_in_frame.CopyFrom(geometry_pb2.Vec3(x=search_ray_end_in_frame[0], y=search_ray_end_in_frame[1],z=search_ray_end_in_frame[2]))


    auto_cmd.hinge_side = hinge
    ...
    #door_command = door_pb2.DoorCommand.Request(auto_push_command=auto_cmd)
...
def execute_open_door(robot, options, image, point, hinge):
    """High level behavior sequence for commanding the robot to open a door."""


    # Tell the robot to walk toward the door.
    manipulation_feedback = walk_to_object_in_image(robot, image, point, options.debug)
    ...
    open_door(robot, image, snapshot, hinge)
...
def open_door_main(options, robot, image, point, hinge):
    """Main function for opening door."""
    try:


        execute_open_door(robot, options, image, point, hinge)
...
def main():
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Show intermediate debug data."
    )
    ...
    hinge = request_manager.hinge_side

    return open_door_main(options, robot, image, point, hinge)
