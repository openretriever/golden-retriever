"""(Adapted from `predicators`) Small utility functions for spot."""
...
from predicators.structs import (  # TODO we will need to integrate some of these types
    Object,
    State,
    Type,
)
...
_robot_type = Type(
    "robot", ["gripper_open_percentage", "x", "y", "z", "qw", "qx", "qy", "qz"]
)
...
_immovable_object_type = Type(
    "immovable",
    list(_base_object_type.feature_names) + ["flat_top_surface"],
    parent=_base_object_type,
)
_container_type = Type(
    "container", list(_movable_object_type.feature_names), parent=_movable_object_type
)
...
def object_to_top_down_geom(
    ...
    put_on_robot_if_held: bool = True,
) -> utils._Geom2D:
    ...
    if (
        put_on_robot_if_held
        and obj.is_instance(_movable_object_type)
        and state.get(obj, "held") > 0.5
    ):
...
def get_graph_nav_dir() -> Path:
    ...
    return upload_dir / CFG.spot_graph_nav_map  # FIXME add this config directly
...
def sample_random_nearby_point_to_move(
    ...
    for _ in range(max_samples):
        ...
        cand_geom = Rectangle.from_center(
            x, y, robot_geom.width, robot_geom.height, rot
        )
...
def spot_pose_to_geom2d(pose: math_helpers.SE3Pose) -> Rectangle:
    ...
    side_length = 0.25
    yaw = pose.rot.to_yaw()
    return Rectangle.from_center(pose.x, pose.y, front_to_back_length, side_length, yaw)
