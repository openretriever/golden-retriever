""""(Adapted from `predicators`) Interface for detecting objects with fiducials or pretrained models.
...
from retriever.robots.spot.spot_perception_structs import (
...
from retriever.robots.spot.spot_utils import get_april_tag_transform, get_graph_nav_dir
...
def rotate_point_in_image(
    ...
    (from: predicators.utils.rotate_point_in_image)
...
def detect_objects_from_april_tags(
    ...
    fiducial_size: float = 44.45,  # CFG.spot_fiducial_size,
    ...
    for apriltag_detection in apriltag_detections:
        ...
        # Look up transform.
        world_object_tform_tag = get_april_tag_transform(obj_id.april_tag_number)
...
def _query_detic_sam(
    ...
    detection_threshold: float = 0.5,  # CFG.spot_vision_detection_threshold
...
def display_camera_detections(artifacts: Dict[str, Any], axes: plt.Axes) -> None:
    ...
    for ax, camera in zip(axes.flat, camera_order, strict=False):
        ax.clear()
        ax.set_title(camera)
        ax.set_xticks([])
        ax.set_yticks([])
...
if __name__ == "__main__":
    ...
    from predicators.spot_utils.perception.cv2_utils import find_color_based_centroid
    ...
    def _run_manual_test(hostname) -> None:
        # Put inside a function to avoid variable scoping issues.
        args = utils.parse_args(
            env_required=False, seed_required=False, approach_required=False
        )
        ...
        april_tag_id: ObjectDetectionID = AprilTagObjectDetectionID(TEST_APRIL_TAG_ID)
        ...
        object_ids: List[ObjectDetectionID] = [
            april_tag_id,
            known_static_id,
        ] + language_ids
        ...
        detections_outfile = Path("../../utils/spot") / "object_detection_artifacts.png"
        no_detections_outfile = Path("../../utils/spot") / "no_detection_artifacts.png"
        visualize_all_artifacts(artifacts, detections_outfile, no_detections_outfile)

    def _run_pythonic_bowl_test(hostname) -> None:
    ...
    _run_manual_test(_hostname)
    _run_pythonic_bowl_test(_hostname)
