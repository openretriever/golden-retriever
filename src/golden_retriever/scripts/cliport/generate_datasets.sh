...
for task in $LANG_TASKS
    do
        python -m retriever.envs.ravens.demos n=1000 task=$task mode=train data_dir=$DATA_DIR disp=$DISP &
        python -m retriever.envs.ravens.demos n=100  task=$task mode=val   data_dir=$DATA_DIR disp=$DISP &
        python -m retriever.envs.ravens.demos n=100  task=$task mode=test  data_dir=$DATA_DIR disp=$DISP
    done
