"""Dataset manifest helpers for retriever_typing.data.v1."""
...
def event_to_row(event: Event[Any], *, episode_id: str) -> dict[str, Any]:
    """Convert an Event into canonical event-table row."""
    return {
...
def event_table_rows(
    ...
    *,
    ...
    """Convert a buffer to canonical replay/export rows."""
...
def build_episode_manifest(
    ...
    *,
    ...
    """Build episode-level manifest from per-stream buffers."""
...
def build_dataset_manifest(
    ...
    *,
    ...
    """Build dataset-level manifest from spec and episodes."""
    ...
    for episode in manifest.episodes:
        for stream_id in episode.stream_ids:
            if stream_id not in spec_streams:
                raise ValueError(
                    f"episode '{episode.episode_id}' references stream '{stream_id}' "
                    "not present in data spec"
                )
